...
void CPU::decode() {
  ...
  switch(opcode) {
    ...
    case 0x04: D(cout << "beq " << regNames[rs] << ", " << regNames[rt] << ", " << pc + (simm << 2));
               stats.countBranch(); stats.registerSrc(rs); stats.registerSrc(rt);
    ...
    case 0x0f: D(cout << "lui " << regNames[rt] << ", " << dec << simm);
               writeDest = true; stats.registerDest(rt);
               aluOp = SHF_L;
               aluSrc1 = simm;
               aluSrc2 = 16;
...
void CPU::printFinalStats() {
  ...
  cout << "Cycles: " << stats.getCycles() << endl
       ...
       << "Mem ops: " << setprecision(1) << 100.0 * stats.getMemOps()/instructions
       << "% of instructions" << endl
       << "Branches: " << 100.0 * stats.getBranches()/instructions 
       << "% of instructions" << endl
       << "  % Taken: " << 100.0 * stats.getTaken()/stats.getBranches() << endl;
}
